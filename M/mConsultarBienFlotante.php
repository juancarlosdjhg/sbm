...
class bien{
		...
		function consultar(){
			...
			if(!$this->nombre_bien==NULL || !$this->nombre_bien==""){
				...
				$SQL="SELECT * FROM bien as b JOIN datos_bien as db on b.id_bien=db.id_bien JOIN sub_categoria_especifica as sce on CAST(db.id_sub_categoria_especifica AS INT)=sce.id_sub_categoria_especifica JOIN subgrupo as sg  on sg.id_subgrupo=CAST(sce.id_subgrupo AS INT) JOIN concepto_de_movimiento as cm on b.id_concepto=cm.id_concepto WHERE db.nombre_bien LIKE '$this->nombre_bien' AND cm.id_tipo_concepto='14'";	
				/*"SELECT * FROM bien as b JOIN datos_bien as db on b.id_bien=db.id_bien JOIN sub_categoria_especifica as sce on CAST(db.id_sub_categoria_especifica AS INT)=sce.id_sub_categoria_especifica JOIN subgrupo as sg  on sg.id_subgrupo=CAST(sce.id_subgrupo AS INT)  JOIN concepto_de_movimiento as cm on b.id_concepto=cm.id_concepto WHERE db.nombre_bien LIKE '$this->nombre_bien' AND cm.id_tipo_concepto='14'";		*/						
