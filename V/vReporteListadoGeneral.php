...
{ 
    ...
    { 
        ...
        $this->Text(52,40,"Listado General de Bienes Registrados en el Sistema"); 
		$this->SetXY(10, 50);
        ...
        $this->SetFillColor(150,180,240); 
             $this->Ln();
              ...
              $this->Cell(30,7,utf8_decode("  Subcategoría"),1,0, 'L', true);
               $this->Cell(35,7,"  Estado del Bien",1,0, 'L', true);
...
$conx=pg_connect($tiracnx);
  ...
  $SQL="SELECT * FROM datos_bien as db JOIN sub_categoria_especifica as sce on CAST(db.id_sub_categoria_especifica AS INT)=sce.id_sub_categoria_especifica JOIN subgrupo as sg  on sg.id_subgrupo=CAST(sce.id_subgrupo AS INT)  join bien on db.id_bien=bien.id_bien join concepto_de_movimiento as cdm on bien.id_concepto=cdm.id_concepto join tipo_concepto as tc on cdm.id_tipo_concepto=tc.id_tipo_concepto ORDER BY sg.id_subgrupo,db.nombre_bien";
        ...
        { 
          ...
          $estado=$row['nombre_tipo_concepto']; 
          ...
          elseif($estado=="Desincorporación"){
            $estado="Desincorporado";
             ...
             $this->Cell(35,5,utf8_decode($nombrebien),1,0, 'L', $color);
              $this->Cell(40,5,$fecha,1,0, 'C', $color);
               $this->Cell(40,5,utf8_decode($serial),1,0, 'L', $color);
                $this->Cell(30,5,utf8_decode($subgrupo),1,0, 'L', $color);
                $this->Cell(35,5,utf8_decode($estado),1,0, 'L', $color);
                 $this->Cell(25,5,$valor." BsF.  ",1,1, 'R', $color);
