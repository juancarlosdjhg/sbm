...
{ 
    ...
    { 
        ...
        $this->Text(45,40,utf8_decode("Listado de Bienes 'Pendientes por Revisión' Registrados en el Sistema")); 
		$this->SetXY(10, 50);
        ...
        $this->SetFillColor(150,180,240); 
             $this->Ln();
              ...
              $this->Cell(30,7,utf8_decode("  Subcategoría"),1,0, 'L', true);
               $this->Cell(35,7,utf8_decode("   S.C. Específica"),1,0, 'L', true);
...
$conx=pg_connect($tiracnx);
  ...
  $SQL="SELECT * FROM datos_bien as db JOIN bien as b on b.id_bien=db.id_bien JOIN sub_categoria_especifica as sce on CAST(db.id_sub_categoria_especifica AS INT)=sce.id_sub_categoria_especifica JOIN subgrupo as sg  on sg.id_subgrupo=CAST(sce.id_subgrupo AS INT) join bien on db.id_bien=bien.id_bien join concepto_de_movimiento as cdm on bien.id_concepto=cdm.id_concepto join tipo_concepto as tc on cdm.id_tipo_concepto=tc.id_tipo_concepto WHERE tc.id_tipo_concepto='16' ORDER BY sg.id_subgrupo,db.nombre_bien";
