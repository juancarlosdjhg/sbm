...
{ 
    ...
    { 
        ...
        $this->Text(45,40,"Listado de Bienes en 'Comodato' Registrados en el Sistema"); 
		$this->SetXY(10, 50);
...
$conx=pg_connect($tiracnx);
  ...
  $SQL="SELECT * FROM datos_bien as db JOIN bien as b on b.id_bien=db.id_bien JOIN sub_categoria_especifica as sce on CAST(db.id_sub_categoria_especifica AS INT)=sce.id_sub_categoria_especifica JOIN subgrupo as sg  on sg.id_subgrupo=CAST(sce.id_subgrupo AS INT) join bien on db.id_bien=bien.id_bien join concepto_de_movimiento as cdm on bien.id_concepto=cdm.id_concepto join tipo_concepto as tc on cdm.id_tipo_concepto=tc.id_tipo_concepto WHERE tc.id_tipo_concepto='18' ORDER BY sg.id_subgrupo,db.nombre_bien";
